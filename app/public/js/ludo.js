...
canvas.height = 750;
canvas.width = 750;
//setting up all pieces positions
let allPiecesePos = {
...
//setting up homeTilePos positions
let homeTilePos = {
...
function diceAction() {
    function rollDice() {
        socket.emit('roll-dice', { room: room_code, id: myid }, function (num) {
            ...
            let spirit = [];
            let canMoveOutFromHome = false;
            let needsExactNumber = false;

            for (let i = 0; i < 4; i++) {
                let piece = PLAYERS[myid].myPieces[i];
                // Check if piece is on the board and can legally move forward by 'num' steps
                if (piece.pos > -1 && piece.pos + num <= 56) {
                    spirit.push(i);
                }
                // Check if piece can move out from home
                if (num == 6 && piece.pos == -1) {
                    canMoveOutFromHome = true;
                }
                // Check if piece needs exact number to reach the center
                if (piece.pos > -1 && piece.pos + num == 56) {
                    needsExactNumber = true;
                }
            }

            let hasValidMove = (spirit.length != 0) || canMoveOutFromHome || needsExactNumber;
            ...
            if (hasValidMove) {
                ...
                canvas.addEventListener('click', function clickHandler(e) {
                    ...
                    for (let i = 0; i < 4; i++) {
                        let piece = PLAYERS[myid].myPieces[i];
                        if (Xp - piece.x < 45 && Xp - piece.x > 0 && Yp - piece.y < 45 && Yp - piece.y > 0) {
                            console.log(i, 'okokokok');
                            if ((spirit.includes(i) || (num == 6 && piece.pos == -1)) && piece.pos + num <= 56) {
                                playerObj['pid'] = i;
                ...
                // No valid moves, pass the dice to the next player
                console.log('19/6/21 no valid move, passing chance');
...
// Helper function to rotate the chance
function chanceRotation(id, num) {
    if (num == 6) {
        console.log('19/6/21 next chance (num == 6)', id);
        return id;
    } else {
        let nextIndex = (MYROOM.indexOf(id) + 1) % MYROOM.length;
        let nextId = MYROOM[nextIndex];
        console.log('19/6/21 next chance', nextId, '\nMYROOM', MYROOM, '\nPrevious chance', id);
        return nextId;
    }
}
...
// }
// //Resets the tooltip text for button
// function outFunc() {
//     var tooltip = document.getElementById("myTooltip");
