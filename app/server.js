...
//---
// Add logging to indicate which instance is serving the request
app.use((req, res, next) => {
    console.log(`Request served by instance running on port ${PORT} - ${req.method} ${req.url}`);
    next();
});
//
///sockets
...
nsp.on('connection',(socket)=>{
    ...
    //---------
    socket.on('WON', (OBJ) => {
        console.log('Player has won:', OBJ);
        delete rooms[OBJ.room];
        nsp.to(OBJ.room).emit('winner', OBJ.id);
    });
